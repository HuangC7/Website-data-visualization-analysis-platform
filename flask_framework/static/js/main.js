...
/*
 *  myChart1 -- 分析统计每一种类的月票数量   ---> echarts柱状图
 *  myChart2 -- 分析统计月票数在5000-10000,10000-15000,15000-20000,以及大于20000的个数   ---> echarts饼状图
 *  myChart3 -- 分析统计每个种类的个数    ---> echarts折线图
 *  myChart4 -- 分析统计每一个月的月票数量总数   ---> echarts饼状图
 *  myChart5 -- 分析统计每个月的都市种类的数量   ---> echarts饼状图
 *  */
...
//获取数据
$.getJSON("static/json/data1.json",function(data1) {
    var d=data1;
    var xlist=[];
    var nanlist=[];
    ...
    myChart1.on('click', function (params) {
        myChart1.dispatchAction({
            type: 'dataZoom',
            startValue: xlist[Math.max(params.dataIndex - zoomSize / 2, 0)],
            endValue: xlist[Math.min(params.dataIndex + zoomSize / 2, nanlist.length - 1)]
        });
    });
});
...
$.getJSON("static/json/data2.json",function(data2){
    var d=data2;
    var nanlist=[];
...
$.getJSON("static/json/data3.json",function(data3){
    var d=data3;
    var nanlist=[];
    var xlist=[];
...
$.getJSON("static/json/data4.json",function(data4){
    var d=data4.月票总数;
    var nanlist=[];
...
$.getJSON("static/json/data5.json",function(data5){
    var d=data5.种类个数;
    var nanlist=[];
